-- =========================================================================
-- Saudi Customs Clearance: Supabase RLS & Auth Trigger
-- Specifically tailored for Prisma-generated schemas (camelCase quoted columns)
-- =========================================================================
...
-- Handles metadata extraction and duplicate conflict resolution
...
CREATE POLICY "Users can update own profile"
  ...
  WITH CHECK (
    -- Prevent non-admins from self-escalating role
...
-- -------------------------------------------------------------------------
-- 6. Invoices Policies (Strictly Enforcing Principle of Least Privilege)
-- Note: Double quotes are required around "createdById" to match Prisma's camelCase column
-- -------------------------------------------------------------------------

-- SELECT: Agents see their own; Admins and Auditors can view all
...
-- INSERT: Agents and Admins can create invoices (createdById must be verified)
...
-- UPDATE: Agents can update their pending/processing invoices; Admins can update any
...
-- Note: Double quotes around "invoiceId" and "createdById" to match Prisma's camelCase columns
-- -------------------------------------------------------------------------

-- SELECT: Accessible if parent invoice is accessible
...
-- INSERT: Only when user owns the parent invoice (or ADMIN)
...
-- UPDATE: Line item editing/HS verification
...
-- DELETE: Only Admins can delete individual line items
...
-- -------------------------------------------------------------------------
-- 8. Saudi Tariff Catalog Policies & Vector Index
-- -------------------------------------------------------------------------
...
-- Tariff management restricted to ADMIN
...
-- Disallow UPDATE and DELETE on audit logs completely
